...
const { generateTestFiles } = require('./lib/generate_test_files');

const GEMINI_API_KEY = process.env.GEMINI_API_KEY || "";

// Startup validation for API key
if (!GEMINI_API_KEY) {
  console.warn('[STARTUP WARNING] GEMINI_API_KEY is not set. Will use rule-based test generation only.');
} else {
  console.log('[STARTUP] GEMINI_API_KEY configured successfully.');
}
...
app.use('/execute', apiLimiter); // Apply rate limiting to execute endpoint
...
function validateTaskPayload(body) {
  ...
  if (body.type !== 'task_assignment') {
    errors.push('Message type must be "task_assignment"');
  }
  ...
  const taskData = body["results/task"];
  if (!taskData || typeof taskData !== 'object') {
    errors.push('results/task object is required');
    return { valid: false, errors };
  }
  
  // Validate code_files_base64 for path traversal
  if (Array.isArray(taskData.code_files_base64)) {
    for (const file of taskData.code_files_base64) {
      if (file.file_path && (file.file_path.includes('..') || path.isAbsolute(file.file_path))) {
        errors.push(`Invalid file path detected: ${file.file_path}. Path traversal not allowed.`);
      }
    }
  }
  
  // Validate git URL format
  if (taskData.git_repo_url && typeof taskData.git_repo_url === 'string') {
    if (!taskData.git_repo_url.match(/^https?:\/\/.+\.git$/i)) {
      errors.push('git_repo_url must be a valid HTTPS .git URL');
    }
  }
...
function sanitizeFilePath(filePath) {
  if (!filePath) return 'file.js';
  // Remove any path traversal attempts
  const sanitized = filePath
    .replace(/\.\./g, '')
    .replace(/^[\/\\]+/, '')
    .replace(/[\/\\]+/g, '/');
  return sanitized || 'file.js';
}
...
app.post('/execute', async (req, res) => {
  ...
  // Validate input
  const validation = validateTaskPayload(incomingMessage);
  if (!validation.valid) {
    return res.status(400).json({
      message_id: `error-${uuidv4()}`,
      status: 'failed',
  ...
  try {
    // --- SMART LTM CHECK (RE-ENABLED) ---
    ltmData = await readLTM();
    taskSignature = await generateTaskSignature(taskData); 
    console.log(`[LTM] Signature: ${taskSignature.substring(0, 16)}...`);
    ...
    const successResponse = createSuccessResponse(
      ...
      // Generate runnable test files for the target framework and attach as ZIP (base64) to response
      try {
        const framework = inferTestFramework(targetLanguage);
        const testFilesMap = generateTestFiles(generated_test_cases, normalizedPayload, framework, targetLanguage);
        ...
        console.log(`[TestGen] Generated ${Object.keys(testFilesMap).length} test files for ${framework}`);
      } catch (e) {
        console.warn('[TestGen] Failed to generate test files:', e && e.message);
      }
    
    // Save to LTM for future cache hits
    await saveToLTM(taskSignature, successResponse["results/task"], ltmData);
    console.log(`[LTM] Saved result to cache. Sig: ${taskSignature.substring(0, 16)}...`);
  ...
  if (Array.isArray(taskData.code_files_base64) && taskData.code_files_base64.length > 0) {
    for (const file of taskData.code_files_base64) {
      try {
        ...
        const safePath = sanitizeFilePath(file.file_path);
        extractedFiles.push({ file_path: safePath, code_snippet: content });
      } catch (e) {}
    }
  }
